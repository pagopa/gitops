variable "io-functions-services-cache" {
  default = {
    repository = {
      organization   = "pagopa"
      name           = "io-functions-services-cache"
      branch_name    = "master"
...
resource "azuredevops_build_definition" "io-functions-services-cache-code-review" {
  ...
  name       = "${var.io-functions-services-cache.repository.name}.code-review"
  ...
  pull_request_trigger {
    ...
    override {
      auto_cancel = false
      branch_filter {
        include = [var.io-functions-services-cache.repository.branch_name]
      }
      path_filter {
        exclude = []
        include = []
  ...
  repository {
    repo_type             = "GitHub"
    repo_id               = "${var.io-functions-services-cache.repository.organization}/${var.io-functions-services-cache.repository.name}"
    branch_name           = var.io-functions-services-cache.repository.branch_name
    yml_path              = "${var.io-functions-services-cache.repository.pipelines_path}/code-review-pipelines.yml"
...
resource "azuredevops_build_definition" "io-functions-services-cache-deploy" {
  ...
  project_id = azuredevops_project.project.id
  name       = "${var.io-functions-services-cache.repository.name}.deploy"
  path       = "\\${var.io-functions-services-cache.repository.name}"
  ...
  repository {
    ...
    repo_id               = "${var.io-functions-services-cache.repository.organization}/${var.io-functions-services-cache.repository.name}"
    branch_name           = var.io-functions-services-cache.repository.branch_name
    yml_path              = "${var.io-functions-services-cache.repository.pipelines_path}/deploy-pipelines.yml"
  ...
  variable {
    ...
    value = var.io-functions-services-cache.pipeline.cache_version_id
  }
  ...
  variable {
    ...
    value = var.io-functions-services-cache.pipeline.production_app_name
  }

  variable {
    name  = "PRODUCTION_RESOURCE_GROUP_NAME"
    value = var.io-functions-services-cache.pipeline.production_resource_group_name
  }
