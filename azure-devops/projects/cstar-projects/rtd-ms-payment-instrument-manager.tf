variable "rtd-ms-payment-instrument-manager" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "rtd-ms-payment-instrument-manager"
      branch_name     = "master"
...
locals {
  # global vars
  rtd-ms-payment-instrument-manager-variables = {
    dockerfile = "DockerfileV1"
  }
  # global secrets
  rtd-ms-payment-instrument-manager-variables_secret = {

  }
  # code_review vars
  rtd-ms-payment-instrument-manager-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.rtd-ms-payment-instrument-manager.repository.organization
    sonarcloud_project_key  = "${var.rtd-ms-payment-instrument-manager.repository.organization}_${var.rtd-ms-payment-instrument-manager.repository.name}"
    sonarcloud_project_name = var.rtd-ms-payment-instrument-manager.repository.name
  }
  # code_review secrets
  rtd-ms-payment-instrument-manager-variables_secret_code_review = {

  }
  # deploy vars
  rtd-ms-payment-instrument-manager-variables_deploy = {
    k8s_image_repository_name            = replace(var.rtd-ms-payment-instrument-manager.repository.name, "-", "")
  ...
  # deploy secrets
  rtd-ms-payment-instrument-manager-variables_secret_deploy = {

  }
}

module "rtd-ms-payment-instrument-manager_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v1.0.0"
  count  = var.rtd-ms-payment-instrument-manager.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.rtd-ms-payment-instrument-manager.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.io-azure-devops-github-pr.id

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.rtd-ms-payment-instrument-manager-variables,
    local.rtd-ms-payment-instrument-manager-variables_code_review,
  )

  variables_secret = merge(
    local.rtd-ms-payment-instrument-manager-variables_secret,
    local.rtd-ms-payment-instrument-manager-variables_secret_code_review,
  )
...
module "rtd-ms-payment-instrument-manager_deploy" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_deploy?ref=v1.0.0"
  count  = var.rtd-ms-payment-instrument-manager.pipeline.enable_deploy == true ? 1 : 0
  ...
  variables = merge(
    local.rtd-ms-payment-instrument-manager-variables,
    local.rtd-ms-payment-instrument-manager-variables_deploy,
  )

  variables_secret = merge(
    local.rtd-ms-payment-instrument-manager-variables_secret,
    local.rtd-ms-payment-instrument-manager-variables_secret_deploy,
  )
