variable "fa-ms-transaction" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "fa-ms-transaction"
      branch_name     = "master"
...
locals {
  # global vars
  fa-ms-transaction-variables = {
    dockerfile = "DockerfileV1"
  }
  # global secrets
  fa-ms-transaction-variables_secret = {

  }
  # code_review vars
  fa-ms-transaction-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.fa-ms-transaction.repository.organization
    sonarcloud_project_key  = "${var.fa-ms-transaction.repository.organization}_${var.fa-ms-transaction.repository.name}"
    sonarcloud_project_name = var.fa-ms-transaction.repository.name
  }
  # code_review secrets
  fa-ms-transaction-variables_secret_code_review = {
  ...
  # deploy secrets
  fa-ms-transaction-variables_secret_deploy = {

  }
}

module "fa-ms-transaction_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v1.0.0"
  count  = var.fa-ms-transaction.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.fa-ms-transaction.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.io-azure-devops-github-pr.id

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.fa-ms-transaction-variables,
    local.fa-ms-transaction-variables_code_review,
  )

  variables_secret = merge(
    local.fa-ms-transaction-variables_secret,
    local.fa-ms-transaction-variables_secret_code_review,
  )
...
module "fa-ms-transaction_deploy" {
  ...
  variables = merge(
    local.fa-ms-transaction-variables,
    local.fa-ms-transaction-variables_deploy,
  )

  variables_secret = merge(
    local.fa-ms-transaction-variables_secret,
    local.fa-ms-transaction-variables_secret_deploy,
  )
