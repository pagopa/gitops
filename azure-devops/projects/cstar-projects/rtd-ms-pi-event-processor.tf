variable "rtd-ms-pi-event-processor" {
  default = {
    repository = {
      organization    = "pagopa"
...
locals {
  # global vars
  rtd-ms-pi-event-processor-variables = {
    dockerfile = "Dockerfile"
  }
  # global secrets
  rtd-ms-pi-event-processor-variables_secret = {

  }
  # code_review vars
  rtd-ms-pi-event-processor-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.rtd-ms-pi-event-processor.repository.organization
    sonarcloud_project_key  = "${var.rtd-ms-pi-event-processor.repository.organization}_${var.rtd-ms-pi-event-processor.repository.name}"
    sonarcloud_project_name = var.rtd-ms-pi-event-processor.repository.name
  }
  # code_review secrets
  rtd-ms-pi-event-processor-variables_secret_code_review = {

  }
  # deploy vars
  rtd-ms-pi-event-processor-variables_deploy = {
    k8s_image_repository_name            = replace(var.rtd-ms-pi-event-processor.repository.name, "-", "")
  ...
  # deploy secrets
  rtd-ms-pi-event-processor-variables_secret_deploy = {

  }
}

module "rtd-ms-pi-event-processor_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v1.0.0"
  count  = var.rtd-ms-pi-event-processor.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.rtd-ms-pi-event-processor.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.io-azure-devops-github-pr.id

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.rtd-ms-pi-event-processor-variables,
    local.rtd-ms-pi-event-processor-variables_code_review,
  )

  variables_secret = merge(
    local.rtd-ms-pi-event-processor-variables_secret,
    local.rtd-ms-pi-event-processor-variables_secret_code_review,
  )
...
module "rtd-ms-pi-event-processor_deploy" {
  ...
  variables = merge(
    local.rtd-ms-pi-event-processor-variables,
    local.rtd-ms-pi-event-processor-variables_deploy,
  )

  variables_secret = merge(
    local.rtd-ms-pi-event-processor-variables_secret,
    local.rtd-ms-pi-event-processor-variables_secret_deploy,
  )
