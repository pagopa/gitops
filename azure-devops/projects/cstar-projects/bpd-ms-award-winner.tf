variable "bpd-ms-award-winner" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "bpd-ms-award-winner"
      branch_name     = "master"
...
locals {
  ...
  # code_review vars
  bpd-ms-award-winner-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.bpd-ms-award-winner.repository.organization
    sonarcloud_project_key  = "${var.bpd-ms-award-winner.repository.organization}_${var.bpd-ms-award-winner.repository.name}"
    sonarcloud_project_name = var.bpd-ms-award-winner.repository.name
  }
  # code_review secrets
  bpd-ms-award-winner-variables_secret_code_review = {
...
module "bpd-ms-award-winner_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v1.0.0"
  count  = var.bpd-ms-award-winner.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.bpd-ms-award-winner.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.io-azure-devops-github-pr.id

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.bpd-ms-award-winner-variables,
    local.bpd-ms-award-winner-variables_code_review,
  )

  variables_secret = merge(
    local.bpd-ms-award-winner-variables_secret,
    local.bpd-ms-award-winner-variables_secret_code_review,
  )
...
module "bpd-ms-award-winner_deploy" {
  ...
  variables = merge(
    local.bpd-ms-award-winner-variables,
    local.bpd-ms-award-winner-variables_deploy,
  )

  variables_secret = merge(
    local.bpd-ms-award-winner-variables_secret,
    local.bpd-ms-award-winner-variables_secret_deploy,
  )
