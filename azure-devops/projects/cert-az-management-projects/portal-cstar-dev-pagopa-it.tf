variable "portal-cstar-dev-pagopa-it" {
  default = {
    ...
    pipeline = {
      enable_cert_az = true
      path           = "cstar"
      name           = "portal.dev.cstar.pagopa.it"
      # common variables to all pipelines
      variables = {
        DO_RENEW_CERT               = "true"
        PRODUCTION_AcmeDirectory    = "LE_PROD"
        PRODUCTION_CertificateNames = "portal.dev.cstar.pagopa.it"
...
locals {
  portal-cstar-dev-pagopa-it-variables = {
    ...
    PRODUCTION_KeyVaultResourceId = "/subscriptions/${module.secrets.values["PAGOPAIT-DEV-CSTAR-SUBSCRIPTION-ID"].value}/resourceGroups/${var.portal-cstar-dev-pagopa-it.pipeline.variables.PRODUCTION_ResourceGroup}/providers/Microsoft.KeyVault/vaults/${var.portal-cstar-dev-pagopa-it.pipeline.variables.PRODUCTION_KeyVault}"
    TEST_AcmeContact              = "NA"
    TEST_AZURE_SUBSCRIPTION       = azuredevops_serviceendpoint_azurerm.DEV-CSTAR.service_endpoint_name
    TEST_KeyVaultResourceId       = "/subscriptions/${module.secrets.values["PAGOPAIT-DEV-CSTAR-SUBSCRIPTION-ID"].value}/resourceGroups/${var.portal-cstar-dev-pagopa-it.pipeline.variables.TEST_ResourceGroup}/providers/Microsoft.KeyVault/vaults/${var.portal-cstar-dev-pagopa-it.pipeline.variables.TEST_KeyVault}"
  }
...
module "portal-cstar-dev-pagopa-it-cert_az" {
  ...
  count  = var.portal-cstar-dev-pagopa-it.pipeline.enable_cert_az == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.portal-cstar-dev-pagopa-it.repository
  name                         = var.portal-cstar-dev-pagopa-it.pipeline.name
  path                         = var.portal-cstar-dev-pagopa-it.pipeline.path
