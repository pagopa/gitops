variable "management-cstar-uat-pagopa-it" {
  default = {
    ...
    pipeline = {
      enable_cert_az = true
      path           = "cstar"
      name           = "management.uat.cstar.pagopa.it"
      # common variables to all pipelines
      variables = {
        DO_RENEW_CERT               = "true"
        PRODUCTION_AcmeDirectory    = "LE_PROD"
        PRODUCTION_CertificateNames = "management.uat.cstar.pagopa.it"
...
locals {
  management-cstar-uat-pagopa-it-variables = {
    ...
    PRODUCTION_KeyVaultResourceId = "/subscriptions/${module.secrets.values["PAGOPAIT-UAT-CSTAR-SUBSCRIPTION-ID"].value}/resourceGroups/${var.management-cstar-uat-pagopa-it.pipeline.variables.PRODUCTION_ResourceGroup}/providers/Microsoft.KeyVault/vaults/${var.management-cstar-uat-pagopa-it.pipeline.variables.PRODUCTION_KeyVault}"
    TEST_AcmeContact              = "NA"
    TEST_AZURE_SUBSCRIPTION       = azuredevops_serviceendpoint_azurerm.UAT-CSTAR.service_endpoint_name
    TEST_KeyVaultResourceId       = "/subscriptions/${module.secrets.values["PAGOPAIT-UAT-CSTAR-SUBSCRIPTION-ID"].value}/resourceGroups/${var.management-cstar-uat-pagopa-it.pipeline.variables.TEST_ResourceGroup}/providers/Microsoft.KeyVault/vaults/${var.management-cstar-uat-pagopa-it.pipeline.variables.TEST_KeyVault}"
  }
...
module "management-cstar-uat-pagopa-it-cert_az" {
  ...
  count  = var.management-cstar-uat-pagopa-it.pipeline.enable_cert_az == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.management-cstar-uat-pagopa-it.repository
  name                         = var.management-cstar-uat-pagopa-it.pipeline.name
  path                         = var.management-cstar-uat-pagopa-it.pipeline.path
