...
locals {
  ...
  tlscert-prod-management-platform-pagopa-it-variables = {
    KEY_VAULT_CERT_NAME          = "${replace(var.tlscert-prod-management-platform-pagopa-it.pipeline.dns_record_name, ".", "-")}-${replace(var.tlscert-prod-management-platform-pagopa-it.pipeline.dns_zone_name, ".", "-")}"
