variable "rtd-transaction-manager" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "rtd-transaction-manager"
      branch_name     = "master"
...
locals {
  # global vars
  rtd-transaction-manager-variables = {
    dockerfile = "DockerfileV1"
  }
  # global secrets
  rtd-transaction-manager-variables_secret = {

  }
  # code_review vars
  rtd-transaction-manager-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.rtd-transaction-manager.repository.organization
    sonarcloud_project_key  = "${var.rtd-transaction-manager.repository.organization}_${var.rtd-transaction-manager.repository.name}"
    sonarcloud_project_name = var.rtd-transaction-manager.repository.name
  }
  # code_review secrets
  rtd-transaction-manager-variables_secret_code_review = {

  }
  # deploy vars
  rtd-transaction-manager-variables_deploy = {
    k8s_image_repository_name           = replace(var.rtd-transaction-manager.repository.name, "-", "")
  ...
  # deploy secrets
  rtd-transaction-manager-variables_secret_deploy = {

  }
}

module "rtd-transaction-manager_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v1.0.0"
  count  = var.rtd-transaction-manager.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.rtd-transaction-manager.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.io-azure-devops-github-pr.id

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.rtd-transaction-manager-variables,
    local.rtd-transaction-manager-variables_code_review,
  )

  variables_secret = merge(
    local.rtd-transaction-manager-variables_secret,
    local.rtd-transaction-manager-variables_secret_code_review,
  )
...
module "rtd-transaction-manager_deploy" {
  ...
  variables = merge(
    local.rtd-transaction-manager-variables,
    local.rtd-transaction-manager-variables_deploy,
  )

  variables_secret = merge(
    local.rtd-transaction-manager-variables_secret,
    local.rtd-transaction-manager-variables_secret_deploy,
  )
