...
locals {
  ...
  # code_review vars
  bpd-ms-citizen-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.bpd-ms-citizen.repository.organization
    sonarcloud_project_key  = "${var.bpd-ms-citizen.repository.organization}_${var.bpd-ms-citizen.repository.name}"
    sonarcloud_project_name = var.bpd-ms-citizen.repository.name
  }
  # code_review secrets
  bpd-ms-citizen-variables_secret_code_review = {

  }
  # deploy vars
  bpd-ms-citizen-variables_deploy = {
    k8s_image_repository_name           = replace(var.bpd-ms-citizen.repository.name, "-", "")
  ...
  # deploy secrets
  bpd-ms-citizen-variables_secret_deploy = {

  }
}

module "bpd-ms-citizen_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v1.0.0"
  count  = var.bpd-ms-citizen.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.bpd-ms-citizen.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.io-azure-devops-github-pr.id

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.bpd-ms-citizen-variables,
    local.bpd-ms-citizen-variables_code_review,
  )

  variables_secret = merge(
    local.bpd-ms-citizen-variables_secret,
    local.bpd-ms-citizen-variables_secret_code_review,
  )
...
module "bpd-ms-citizen_deploy" {
  ...
  variables = merge(
    local.bpd-ms-citizen-variables,
    local.bpd-ms-citizen-variables_deploy,
  )

  variables_secret = merge(
    local.bpd-ms-citizen-variables_secret,
    local.bpd-ms-citizen-variables_secret_deploy,
  )
