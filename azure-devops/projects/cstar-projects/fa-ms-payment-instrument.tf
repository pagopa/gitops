variable "fa-ms-payment-instrument" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "fa-ms-payment-instrument"
      branch_name     = "master"
...
locals {
  # global vars
  fa-ms-payment-instrument-variables = {
    dockerfile = "DockerfileV1"
  }
  # global secrets
  fa-ms-payment-instrument-variables_secret = {

  }
  # code_review vars
  fa-ms-payment-instrument-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.fa-ms-payment-instrument.repository.organization
    sonarcloud_project_key  = "${var.fa-ms-payment-instrument.repository.organization}_${var.fa-ms-payment-instrument.repository.name}"
    sonarcloud_project_name = var.fa-ms-payment-instrument.repository.name
  }
  # code_review secrets
  fa-ms-payment-instrument-variables_secret_code_review = {

  }
  # deploy vars
  fa-ms-payment-instrument-variables_deploy = {
    k8s_image_repository_name            = replace(var.fa-ms-payment-instrument.repository.name, "-", "")
    deploy_namespace                     = "fa"
  ...
  # deploy secrets
  fa-ms-payment-instrument-variables_secret_deploy = {

  }
}

module "fa-ms-payment-instrument_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v1.0.0"
  count  = var.fa-ms-payment-instrument.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.fa-ms-payment-instrument.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.io-azure-devops-github-pr.id

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.fa-ms-payment-instrument-variables,
    local.fa-ms-payment-instrument-variables_code_review,
  )

  variables_secret = merge(
    local.fa-ms-payment-instrument-variables_secret,
    local.fa-ms-payment-instrument-variables_secret_code_review,
  )
...
module "fa-ms-payment-instrument_deploy" {
  ...
  variables = merge(
    local.fa-ms-payment-instrument-variables,
    local.fa-ms-payment-instrument-variables_deploy,
  )

  variables_secret = merge(
    local.fa-ms-payment-instrument-variables_secret,
    local.fa-ms-payment-instrument-variables_secret_deploy,
  )
