variable "fa-ms-invoice-manager" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "fa-ms-invoice-manager"
      branch_name     = "master"
...
locals {
  # global vars
  fa-ms-invoice-manager-variables = {
    dockerfile = "DockerfileV1"
  }
  # global secrets
  fa-ms-invoice-manager-variables_secret = {

  }
  # code_review vars
  fa-ms-invoice-manager-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.fa-ms-invoice-manager.repository.organization
    sonarcloud_project_key  = "${var.fa-ms-invoice-manager.repository.organization}_${var.fa-ms-invoice-manager.repository.name}"
    sonarcloud_project_name = var.fa-ms-invoice-manager.repository.name
  }
  # code_review secrets
  fa-ms-invoice-manager-variables_secret_code_review = {

  }
  # deploy vars
  fa-ms-invoice-manager-variables_deploy = {
    k8s_image_repository_name            = replace(var.fa-ms-invoice-manager.repository.name, "-", "")
    deploy_namespace                     = "fa"
  ...
  # deploy secrets
  fa-ms-invoice-manager-variables_secret_deploy = {

  }
}

module "fa-ms-invoice-manager_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v1.0.0"
  count  = var.fa-ms-invoice-manager.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.fa-ms-invoice-manager.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.io-azure-devops-github-pr.id

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.fa-ms-invoice-manager-variables,
    local.fa-ms-invoice-manager-variables_code_review,
  )

  variables_secret = merge(
    local.fa-ms-invoice-manager-variables_secret,
    local.fa-ms-invoice-manager-variables_secret_code_review,
  )
...
module "fa-ms-invoice-manager_deploy" {
  ...
  variables = merge(
    local.fa-ms-invoice-manager-variables,
    local.fa-ms-invoice-manager-variables_deploy,
  )

  variables_secret = merge(
    local.fa-ms-invoice-manager-variables_secret,
    local.fa-ms-invoice-manager-variables_secret_deploy,
  )
