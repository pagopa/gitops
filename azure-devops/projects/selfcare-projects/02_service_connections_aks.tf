...
resource "azuredevops_serviceendpoint_kubernetes" "selfcare-aks-dev" {
  ...
  apiserver_url         = module.selfcare_dev_secrets.values["selc-d-aks-apiserver-url"].value
  authorization_type    = "ServiceAccount"
  service_account {
    # base64 values
    token   = module.selfcare_dev_secrets.values["aks-azure-devops-sa-token"].value
    ca_cert = module.selfcare_dev_secrets.values["aks-azure-devops-sa-cacrt"].value
  }
}
