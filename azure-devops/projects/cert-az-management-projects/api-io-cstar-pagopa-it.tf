variable "api-io-cstar-pagopa-it" {
  default = {
    ...
    pipeline = {
      enable_cert_az = true
      path           = "cstar"
      name           = "api-io.cstar.pagopa.it"
      # common variables to all pipelines
      variables = {
        DO_RENEW_CERT               = "true"
        PRODUCTION_AcmeDirectory    = "LE_PROD"
        PRODUCTION_CertificateNames = "api-io.cstar.pagopa.it"
...
locals {
  api-io-cstar-pagopa-it-variables = {
    ...
    PRODUCTION_KeyVaultResourceId = "/subscriptions/${module.secrets.values["PAGOPAIT-PROD-CSTAR-SUBSCRIPTION-ID"].value}/resourceGroups/${var.api-io-cstar-pagopa-it.pipeline.variables.PRODUCTION_ResourceGroup}/providers/Microsoft.KeyVault/vaults/${var.api-io-cstar-pagopa-it.pipeline.variables.PRODUCTION_KeyVault}"
    TEST_AcmeContact              = "NA"
    TEST_AZURE_SUBSCRIPTION       = azuredevops_serviceendpoint_azurerm.PROD-CSTAR.service_endpoint_name
    TEST_KeyVaultResourceId       = "/subscriptions/${module.secrets.values["PAGOPAIT-PROD-CSTAR-SUBSCRIPTION-ID"].value}/resourceGroups/${var.api-io-cstar-pagopa-it.pipeline.variables.TEST_ResourceGroup}/providers/Microsoft.KeyVault/vaults/${var.api-io-cstar-pagopa-it.pipeline.variables.TEST_KeyVault}"
  }
...
module "api-io-cstar-pagopa-it-cert_az" {
  ...
  count  = var.api-io-cstar-pagopa-it.pipeline.enable_cert_az == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.api-io-cstar-pagopa-it.repository
  name                         = var.api-io-cstar-pagopa-it.pipeline.name
  path                         = var.api-io-cstar-pagopa-it.pipeline.path
