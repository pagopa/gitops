variable "bpd-ms-notification-manager" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "bpd-ms-notification-manager"
      branch_name     = "master"
...
locals {
  ...
  # code_review vars
  bpd-ms-notification-manager-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.bpd-ms-notification-manager.repository.organization
    sonarcloud_project_key  = "${var.bpd-ms-notification-manager.repository.organization}_${var.bpd-ms-notification-manager.repository.name}"
    sonarcloud_project_name = var.bpd-ms-notification-manager.repository.name
  }
  # code_review secrets
  bpd-ms-notification-manager-variables_secret_code_review = {

  }
  # deploy vars
  bpd-ms-notification-manager-variables_deploy = {
    k8s_image_repository_name            = replace(var.bpd-ms-notification-manager.repository.name, "-", "")
  ...
  # deploy secrets
  bpd-ms-notification-manager-variables_secret_deploy = {

  }
}

module "bpd-ms-notification-manager_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v1.0.0"
  count  = var.bpd-ms-notification-manager.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.bpd-ms-notification-manager.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.io-azure-devops-github-pr.id

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.bpd-ms-notification-manager-variables,
    local.bpd-ms-notification-manager-variables_code_review,
  )

  variables_secret = merge(
    local.bpd-ms-notification-manager-variables_secret,
    local.bpd-ms-notification-manager-variables_secret_code_review,
  )
...
module "bpd-ms-notification-manager_deploy" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_deploy?ref=v1.0.0"
  count  = var.bpd-ms-notification-manager.pipeline.enable_deploy == true ? 1 : 0
  ...
  variables = merge(
    local.bpd-ms-notification-manager-variables,
    local.bpd-ms-notification-manager-variables_deploy,
  )

  variables_secret = merge(
    local.bpd-ms-notification-manager-variables_secret,
    local.bpd-ms-notification-manager-variables_secret_deploy,
  )
