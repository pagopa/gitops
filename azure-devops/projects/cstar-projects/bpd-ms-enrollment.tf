variable "bpd-ms-enrollment" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "bpd-ms-enrollment"
      branch_name     = "master"
...
locals {
  ...
  # code_review vars
  bpd-ms-enrollment-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.bpd-ms-enrollment.repository.organization
    sonarcloud_project_key  = "${var.bpd-ms-enrollment.repository.organization}_${var.bpd-ms-enrollment.repository.name}"
    sonarcloud_project_name = var.bpd-ms-enrollment.repository.name
  }
  # code_review secrets
  bpd-ms-enrollment-variables_secret_code_review = {

  }
  # deploy vars
  bpd-ms-enrollment-variables_deploy = {
    k8s_image_repository_name           = replace(var.bpd-ms-enrollment.repository.name, "-", "")
  ...
  # deploy secrets
  bpd-ms-enrollment-variables_secret_deploy = {

  }
}

module "bpd-ms-enrollment_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v1.0.0"
  count  = var.bpd-ms-enrollment.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.bpd-ms-enrollment.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.io-azure-devops-github-pr.id

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.bpd-ms-enrollment-variables,
    local.bpd-ms-enrollment-variables_code_review,
  )

  variables_secret = merge(
    local.bpd-ms-enrollment-variables_secret,
    local.bpd-ms-enrollment-variables_secret_code_review,
  )
...
module "bpd-ms-enrollment_deploy" {
  ...
  variables = merge(
    local.bpd-ms-enrollment-variables,
    local.bpd-ms-enrollment-variables_deploy,
  )

  variables_secret = merge(
    local.bpd-ms-enrollment-variables_secret,
    local.bpd-ms-enrollment-variables_secret_deploy,
  )
