...
locals {
  ...
  tlscert-uat-checkout-pagopa-it-variables = {
    KEY_VAULT_CERT_NAME          = "${replace(var.tlscert-uat-checkout-pagopa-it.pipeline.dns_record_name, ".", "-")}-${replace(var.tlscert-uat-checkout-pagopa-it.pipeline.dns_zone_name, ".", "-")}"
