variable "tlscert-uat-uat-selfcare-pagopa-it" {
  default = {
    repository = {
      organization   = "pagopa"
...
locals {
  tlscert-uat-uat-selfcare-pagopa-it = {
  ...
  tlscert-uat-uat-selfcare-pagopa-it-variables = {
    KEY_VAULT_CERT_NAME          = "${replace(var.tlscert-uat-uat-selfcare-pagopa-it.pipeline.dns_record_name, ".", "-")}-${replace(var.tlscert-uat-uat-selfcare-pagopa-it.pipeline.dns_zone_name, ".", "-")}"
    KEY_VAULT_SERVICE_CONNECTION = module.UAT-SELFCARE-TLS-CERT-SERVICE-CONN.service_endpoint_name
  }
  tlscert-uat-uat-selfcare-pagopa-it-variables_secret = {
  }
}
...
module "tlscert-uat-uat-selfcare-pagopa-it-cert_az" {
  ...
  repository                   = var.tlscert-uat-uat-selfcare-pagopa-it.repository
  ...
  dns_zone_resource_group = var.tlscert-uat-uat-selfcare-pagopa-it.pipeline.dns_zone_resource_group
  tenant_id               = local.tlscert-uat-uat-selfcare-pagopa-it.tenant_id
  subscription_name       = local.tlscert-uat-uat-selfcare-pagopa-it.subscription_name
  subscription_id         = local.tlscert-uat-uat-selfcare-pagopa-it.subscription_id
  ...
  variables = merge(
    var.tlscert-uat-uat-selfcare-pagopa-it.pipeline.variables,
    local.tlscert-uat-uat-selfcare-pagopa-it-variables,
  )

  variables_secret = merge(
    var.tlscert-uat-uat-selfcare-pagopa-it.pipeline.variables_secret,
    local.tlscert-uat-uat-selfcare-pagopa-it-variables_secret,
  )
