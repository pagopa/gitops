...
locals {
  ...
  tlscert-prod-portal-platform-pagopa-it-variables = {
    KEY_VAULT_CERT_NAME          = "${replace(var.tlscert-prod-portal-platform-pagopa-it.pipeline.dns_record_name, ".", "-")}-${replace(var.tlscert-prod-portal-platform-pagopa-it.pipeline.dns_zone_name, ".", "-")}"
