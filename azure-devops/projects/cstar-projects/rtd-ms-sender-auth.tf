...
locals {
  # global vars
  rtd-ms-sender-auth-variables = {
    dockerfile = "Dockerfile"
  }
  # global secrets
  rtd-ms-sender-auth-variables_secret = {

  }
  # code_review vars
  rtd-ms-sender-auth-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.rtd-ms-sender-auth.repository.organization
    sonarcloud_project_key  = "${var.rtd-ms-sender-auth.repository.organization}_${var.rtd-ms-sender-auth.repository.name}"
    sonarcloud_project_name = var.rtd-ms-sender-auth.repository.name
  }
  # code_review secrets
  rtd-ms-sender-auth-variables_secret_code_review = {

  }
  # deploy vars
  rtd-ms-sender-auth-variables_deploy = {
    k8s_image_repository_name            = replace(var.rtd-ms-sender-auth.repository.name, "-", "")
  ...
  # deploy secrets
  rtd-ms-sender-auth-variables_secret_deploy = {

  }
}

module "rtd-ms-sender-auth_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v1.0.0"
  count  = var.rtd-ms-sender-auth.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.rtd-ms-sender-auth.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.io-azure-devops-github-pr.id

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.rtd-ms-sender-auth-variables,
    local.rtd-ms-sender-auth-variables_code_review,
  )

  variables_secret = merge(
    local.rtd-ms-sender-auth-variables_secret,
    local.rtd-ms-sender-auth-variables_secret_code_review,
  )
...
module "rtd-ms-sender-auth_deploy" {
  ...
  variables = merge(
    local.rtd-ms-sender-auth-variables,
    local.rtd-ms-sender-auth-variables_deploy,
  )

  variables_secret = merge(
    local.rtd-ms-sender-auth-variables_secret,
    local.rtd-ms-sender-auth-variables_secret_deploy,
  )
