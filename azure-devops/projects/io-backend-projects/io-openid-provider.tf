...
resource "azuredevops_build_definition" "io-openid-provider-deploy" {
  ...
  project_id = azuredevops_project.project.id
  name       = "${var.io-openid-provider.repository.name}.deploy"
  path       = "\\${var.io-openid-provider.repository.name}"
  ...
  repository {
    ...
    repo_id               = "${var.io-openid-provider.repository.organization}/${var.io-openid-provider.repository.name}"
    branch_name           = var.io-openid-provider.repository.branch_name
    yml_path              = "${var.io-openid-provider.repository.pipelines_path}/deploy-pipelines.yml"
  ...
  variable {
    ...
    value = var.io-openid-provider.pipeline.cache_version_id
  }
...
resource "azuredevops_resource_authorization" "io-openid-provider-deploy-github-ro-auth" {
  depends_on = [azuredevops_serviceendpoint_github.pagopa, azuredevops_build_definition.io-openid-provider-deploy, azuredevops_project.project]

  project_id    = azuredevops_project.project.id
  resource_id   = azuredevops_serviceendpoint_github.pagopa.id
  definition_id = azuredevops_build_definition.io-openid-provider-deploy.id
  authorized    = true
  type          = "endpoint"
}
...
resource "azuredevops_resource_authorization" "io-openid-provider-deploy-github-rw-auth" {
  depends_on = [azuredevops_serviceendpoint_github.io-azure-devops-github-rw, azuredevops_build_definition.io-openid-provider-deploy, azuredevops_project.project]
...
resource "azuredevops_resource_authorization" "io-openid-provider-deploy-azurerm-PROD-IO-auth" {
  depends_on = [azuredevops_serviceendpoint_azurerm.PROD-IO, azuredevops_build_definition.io-openid-provider-deploy, time_sleep.wait]
...
resource "azuredevops_resource_authorization" "io-openid-provider-deploy-npm-auth" {
  depends_on = [azuredevops_serviceendpoint_npm.pagopa-npm-bot, azuredevops_build_definition.io-openid-provider-deploy, time_sleep.wait]
