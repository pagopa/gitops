variable "bpd-ms-transaction-error-manager" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "bpd-ms-transaction-error-manager"
      branch_name     = "master"
...
locals {
  ...
  # code_review vars
  bpd-ms-transaction-error-manager-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.bpd-ms-transaction-error-manager.repository.organization
    sonarcloud_project_key  = "${var.bpd-ms-transaction-error-manager.repository.organization}_${var.bpd-ms-transaction-error-manager.repository.name}"
    sonarcloud_project_name = var.bpd-ms-transaction-error-manager.repository.name
  }
  # code_review secrets
  bpd-ms-transaction-error-manager-variables_secret_code_review = {

  }
  # deploy vars
  bpd-ms-transaction-error-manager-variables_deploy = {
    k8s_image_repository_name           = replace(var.bpd-ms-transaction-error-manager.repository.name, "-", "")
  ...
  # deploy secrets
  bpd-ms-transaction-error-manager-variables_secret_deploy = {

  }
}

module "bpd-ms-transaction-error-manager_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v1.0.0"
  count  = var.bpd-ms-transaction-error-manager.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = azuredevops_project.project.id
  repository                   = var.bpd-ms-transaction-error-manager.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.io-azure-devops-github-pr.id

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.bpd-ms-transaction-error-manager-variables,
    local.bpd-ms-transaction-error-manager-variables_code_review,
  )

  variables_secret = merge(
    local.bpd-ms-transaction-error-manager-variables_secret,
    local.bpd-ms-transaction-error-manager-variables_secret_code_review,
  )
...
module "bpd-ms-transaction-error-manager_deploy" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_deploy?ref=v1.0.0"
  count  = var.bpd-ms-transaction-error-manager.pipeline.enable_deploy == true ? 1 : 0
  ...
  variables = merge(
    local.bpd-ms-transaction-error-manager-variables,
    local.bpd-ms-transaction-error-manager-variables_deploy,
  )

  variables_secret = merge(
    local.bpd-ms-transaction-error-manager-variables_secret,
    local.bpd-ms-transaction-error-manager-variables_secret_deploy,
  )
